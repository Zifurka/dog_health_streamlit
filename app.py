...
with col1:
    ...
    annual_vet = st.slider("Визитов к ветеринару в год", 0.0, 10.0, 1.0, key="annual_vet")
    daily_activity = st.selectbox("Уровень активности в день", ['Low', 'Moderate', 'Active', 'Very Active'], key="daily_activity")

with col2:
    diet = st.selectbox("Рацион питания", df['Diet'].dropna().unique(), key="diet")
    medications = st.selectbox("Лекарства", df['Medications'].dropna().unique(), key="medications")
    seizures = st.selectbox("Судороги", df['Seizures'].dropna().unique(), key="seizures")
    spay_neuter = st.selectbox("Стерилизация/Кастрация", df['Spay/Neuter Status'].dropna().unique(), key="spay_neuter")
...
with col1:
    ...
    daily_activity = st.selectbox("Уровень активности в день", ['Low', 'Moderate', 'Active', 'Very Active'])
